...
import { COURSES, courseOfTee, customTeesFor, searchCourses, teeById, ES_DIRECTORY, PT_DIRECTORY, UK_DIRECTORY } from "@/lib/courses";
    ...
    return (
      <Shell society={society}>
        <PickerHead onBack={() => { setCourseFor(null); setCourseQ(""); }}
                    placeholder="Search UK, Spain & Portugal courses" q={courseQ} setQ={setCourseQ} autoFocus />
        <div className="grid gap-2 rise">
          {!courseQ.trim() && (
            <p className="label !normal-case !tracking-normal">
              Every UK course is in here — {UK_DIRECTORY.length.toLocaleString("en-GB")} across
              England, Scotland, Wales and Northern Ireland — plus {ES_DIRECTORY.length + PT_DIRECTORY.length} across Spain and
